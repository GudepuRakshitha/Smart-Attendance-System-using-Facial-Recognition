...
def initialize_attendance_file():
    ...
    If the file exists, it loads it; otherwise, it creates a new one with headers.
    """
...
def mark_attendance(attendance_df, roll_number):
    ...
    # Check if attendance for this roll number on this date is already marked
    if not ((attendance_df['Roll Number'] == roll_number) & (attendance_df['Date'] == date_str)).any():
        new_entry = {'Roll Number': roll_number, 'Date': date_str, 'Time': time_str}
        attendance_df = attendance_df.append(new_entry, ignore_index=True)
...
def main():
    ...
    # Set to keep track of already marked attendance in this session
    marked_roll_numbers = set(attendance_df['Roll Number'].tolist())
        ...
        while True:
            ...
            for face_encoding, face_location in zip(face_encodings, face_locations):
                ...
                if matches[best_match_index]:
                    ...
                    # Optionally handle unrecognized faces
                    top, right, bottom, left = face_location
                    top *= int(1/FRAME_RESIZE_SCALE)
                    right *= int(1/FRAME_RESIZE_SCALE)
                    bottom *= int(1/FRAME_RESIZE_SCALE)
